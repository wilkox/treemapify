...
is used with the `subgroup`, `subgroup2` or `subgroup3` aesthetic to
subgroup treemap tiles, `geom_treemap_subgroup_text()`,
`geom_treemap_subgroup2_text()` or `geom_treemap_subgroup3_text()` can
be used to add a text label to each subgroup at the appropriate level.
...
- level:
  ...
  One of 'subgroup', 'subgroup2' or 'subgroup3', giving the subgrouping
  level for which to draw text labels. It is recommended to use the
  aliases `geom_treemap_subgroup2_text()` and
  `geom_treemap_subgroup3_text()` instead of this argument.

## Details

`geom_treemap_subgroup_text()` geoms require `area`, `label` and
`subgroup` (or `subgroup2`, `subgroup3`) aesthetics. Several other
standard 'ggplot2' aesthetics are supported (see Aesthetics).

`geom_treemap_subgroup_text()` geoms use the 'ggfittext' package to fit
