...
All the treemapify treemaps support a ‘fixed’ option that will cause the tiles
to be laid out in an order determined by the order of observations in the data
frame. While this can result in a less aesthetically pleasing and readable
treemap, it allows you to create animated treemaps with the help of the
...
usvisitors_split <- split(usvisitors, usvisitors$year)

usvisitors_tweened <- tweenr::tween_states(
  usvisitors_split,
  tweenlength = 40,
  statelength = 100,
  ease = 'cubic-in-out',
...
animated_plot <- ggplot(usvisitors_tweened, aes(
  area = visitors,
  fill = average_stay_nights,
  label = subcategory,
  subgroup = category,
  frame = .frame
  )) +
  geom_treemap(fixed = T) +
  geom_treemap_text(reflow = T, fixed = T) +
  geom_treemap_subgroup_border(fixed = T) +
  geom_treemap_subgroup_text(place = "centre", alpha = 0.5, grow = T, fixed = T)

animation::ani.options(interval = 1/15)
gganimate(animated_plot, title_frame = F, ani.width = 400, ani.height = 400)
```
...
The usvisitors dataset is from [Tourism Research
Australia](https://www.tra.gov.au/Research/International-visitors-to-Australia/international-tourism-statistics).
